...
namespace StockPro.Inventory.Services
{
    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<ProductResponseDto> CreateProductAsync(ProductCreateDto dto)
        {
            ...
            // Business Rule: Barcode unique if present
        ...
        public async Task<ProductResponseDto?> UpdateProductAsync(Guid id, ProductUpdateDto dto)
        {
            ...
            // Business Rule: SKU unique (except itself)
            if (await _context.Products.AnyAsync(p => p.SKU == dto.SKU && p.ProductId != id))
                throw new InvalidOperationException("SKU already exists.");

            // Business Rule: Barcode unique (except itself)
        ...
        public async Task<bool> DeleteProductAsync(Guid id)
        {
            ...
            // Business Rule: Prevent deletion if referenced (Mocked for now)
            // In a real microservice, we would check other tables/services.
            // For now, we assume no references exists until UC3/4/5 are built.
